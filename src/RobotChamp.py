...
class APP:
    def __init__(self):
        ...
        self.app.bind("<F5>",self.change5min) #更改時長5分鐘
        self.app.bind("<F12>",self.changeTestmin) #更改時長1秒
    # 標題 row0 ============================================
        self.t_lb=tk.Label(text="機器人足球錦標賽",font="微軟正黑體 45 bold",bg="black",fg="orange")
    ...
    # 血量 row3 ============================================
        ...
        for i in range(21):
            exec(f"self.img=Image.open('soccer{i}.jpg')")
            exec(f"self.h{i}_img=ImageTk.PhotoImage(self.img)")
        
        self.rh_btn=tk.Button(image=self.h0_img, bg="black", bd=0, cursor='hand2', command=lambda:self.soccer_count("r"))
        self.rh_btn.grid(row=3,column=0,columnspan=4)

        self.bh_btn=tk.Button(image=self.h0_img, bg="black", bd=0, cursor='hand2', command=lambda:self.soccer_count("b"))
        self.bh_btn.grid(row=3,column=9,columnspan=4)
    # 黃牌 row4 ============================================
        self.ryc=0
        self.byc=0
    ...
    def changeTestmin(self,event):
        self.clk_lb.config(text="0:01",fg="green")
        self.clock=1.9
        self.clk=self.clock

    def change5min(self,event):
        self.clk_lb.config(text="5:00",fg="green")
        self.clock=300.9
        self.clk=self.clock
    ...
    def soccer_count(self, c):
        exec(f"self.{c}h+=1")
        exec(f"if self.{c}h>20 : self.{c}h=0")
    ...
    def start(self):
        ...
        if self.clk>0:
            ...
            if self.rh > self.bh:
                ...
                self.alarm_win("r")
            elif self.rh < self.bh:
                self.clk_lb.config(text="Time's Up!")
                self.alarm_win("b")
            else:
                self.clk_lb.config(text="平手!!!")
            self.clk=self.clock
